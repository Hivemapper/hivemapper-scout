...
export const thumbnailWrapper = () =>
  cn(
    { "flex flex-col cursor-pointer": !twStore.get() },
    "hm-thumbnail-wrapper",
  );
export const thumbnailImgSection = () =>
  cn(
    { "flex w-full aspect-w-2 aspect-h-1": !twStore.get() },
...
export const thumbnailDate = () =>
  cn(
    { "text-primary text-sm font-medium tracking-normal mt-1": !twStore.get() },
    "hm-thumbnail-date",
  );
export const thumbnailPrettyDate = () =>
  cn(
