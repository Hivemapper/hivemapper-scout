...
export const monthDayTime = (time: string | Date) => {
  ...
  const { value: year } = formattedDateParts.find(part => part.type === "year") || {};
  const { value: month } = formattedDateParts.find(part => part.type === "month") || {};
  const { value: day } = formattedDateParts.find(part => part.type === "day") || {};
  const { value: hour } = formattedDateParts.find(part => part.type === "hour") || {};
  const { value: minute } = formattedDateParts.find(part => part.type === "minute") || {};
  const { value: dayPeriod } = formattedDateParts.find(part => part.type === "dayPeriod") || {};
